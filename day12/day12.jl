...
# data_loc = "day12_test.txt"
data_loc = "day12_input.txt"


# don't need to transpose, because the CartesianIndex is (y, x)
# heightmap = transpose(map(c -> Int(c), reduce(hcat, collect.(readlines(data_loc))))) 
heightmap = map(c -> Int(c), reduce(hcat, collect.(readlines(data_loc))))
